...
parser.add_argument('--cfg', type=str, required=False, help='experiment configure file name', default='detector/yolo/256x192_res50_lr1e-3_1x.yaml')
parser.add_argument('--checkpoint', type=str, required=False, help='checkpoint file name', default='detector/yolo/fast_res50_256x192.pth')
...
parser.add_argument('--outdir', dest='outputpath', help='output-directory', default="output/images/")
...
parser.add_argument('--posebatch', type=int, default=80, help='pose estimation maximum batch size PER GPU')
...
if __name__ == "__main__":
    ...
    if mode == 'webcam':
        # get_detector函数是在detector/apis.py里面有定义的
    ...
    # Load the model of pose estimation
    ...
    try:
        for i in im_names_desc:
            start_time = getTime()
            with torch.no_grad():
                # inps,[1,3,256,192]的矩阵,我姑且理解为准备放进sppe模型的正则化后的一个人的图像
                # orig_img,[720,1280,3]的矩阵,看过了,是输入的原图像
                # im_name,字符串,图像名,如果是视频的话就按顺序来给0123.jpg
                # boxes,看过了,这是一个框出来的人的bbox,每一个元素的四个值分别是ymin,xmin,ymax,xmax,检查过的啦
                # scores,一个评分
                # ids,应该是图像id号
                # cropped_boxes,这个也是一个bbox,好像跟原boxes相比,是做了test_transform
