...
def validate(m, opt, heatmap_to_coord, batch_size=20):
    ...
    det_loader = torch.utils.data.DataLoader(det_dataset, batch_size=batch_size, shuffle=False, num_workers=20, drop_last=False)
    kpt_json = []
...
def validate_gt(m, opt, cfg, heatmap_to_coord, batch_size=20):
    ...
    gt_val_loader = torch.utils.data.DataLoader(gt_val_dataset, batch_size=batch_size, shuffle=False, num_workers=20, drop_last=False)
    kpt_json = []
    m.eval()
