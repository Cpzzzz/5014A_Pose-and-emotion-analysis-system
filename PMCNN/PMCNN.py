...
def train_epoch(model, loader, optimizer, epoch, n_epochs, print_freq=1):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if batch_idx % print_freq == 0:
            res = '\t'.join(['Epoch: [%d/%d]' % (epoch + 1, n_epochs), 'Iter: [%d/%d]' % (batch_idx + 1, len(loader)), 'Loss %.4f (%.4f)' % (losses.val, losses.avg),
...
def train(model, train_set, test_set, n_epochs=300, batch_size=64, lr=0.1, wd=0.0001, momentum=0.9, seed=None):
    ...
    for epoch in range(n_epochs):
        ...
        if test_error < best_error:
            ...
            torch.save(model, 'training/PMCNN/{}_{}.pth'.format(epoch, best_error))
...
if __name__ == '__main__':
    ...
    train_transforms = transforms.Compose([transforms.RandomHorizontalFlip(), transforms.ToTensor(), transforms.Normalize(mean=mean, std=stdv), ])
    test_transforms = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=stdv), ])

    train_set = datasets.ImageFolder('data/fer2013images/train', transform=train_transforms)
    test_set = datasets.ImageFolder('data/fer2013images/valid', transform=test_transforms)
