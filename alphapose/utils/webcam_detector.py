...
class WebCamDetectionLoader():
    ...
    def start(self):
        # 这个webcam_detector写得就没有detector好,因为明明是分来的步骤全部都写到frame_preprocess里面去了,搞得逻辑非常的不清晰,不好了解
        # 不过也许跟webcam不能超前读取有关,只能读到一张处理一张.这样看来还是video的detector比较好理解和使用一点
    ...
    def frame_preprocess(self):
        ...
        for i in count():
            ...
            if not self.pose_queue.full():
                ...
                # 下面开始来对处理好的图片进行操作，先是用image_detection返回一下render坐标，再用image_postprocess处理，这个还不懂？？？？？？？？？？？？？？？？？
                with torch.no_grad():
    ...
    def image_detection(self, inputs):
        ...
        with torch.no_grad():
            # 这里下面的images_detection好像就是整个地方的关键，看注释是只会把人的留下，dets就是检测之后返回的结果
