...
def vis_frame(frame, im_res, opt, format='coco'):
    ...
    if kp_num == 17:
        if format == 'coco':
            l_pair = [
                # (0, 1), (0, 2), (1, 3), (2, 4),  # Head,因为要做表情识别所以这里脸先不画
                (5, 6), (5, 7), (7, 9), (6, 8), (8, 10),
                (17, 11), (17, 12),  # Body
                (11, 13), (12, 14), (13, 15), (14, 16)
            ]
    ...
    img = frame.copy()
    height, width = img.shape[:2]
    # print(im_res['imgname'])
    for human in im_res['result']:
        # 这里面取出来的human就是一个一个提取好的人了
        part_line = {}
        kp_preds = human['keypoints']
        kp_scores = human['kp_score']
        if kp_num == 17:
            # 17个点，依次是：鼻子、右眼、左眼、右耳、左耳、右肩、左肩、右肘、左肘、右腕、左腕、右髋、左髋、右膝、左膝、右踝、左踝
            # 下面这个第五和第六取平均就是右肩和左肩的中间，相当于把这个点加在kp_preds的最后面
            kp_preds = torch.cat((kp_preds, torch.unsqueeze((kp_preds[5, :] + kp_preds[6, :]) / 2, 0)))
            kp_scores = torch.cat((kp_scores, torch.unsqueeze((kp_scores[5, :] + kp_scores[6, :]) / 2, 0)))
            color = RED
        ...
        l_x_dis = abs(LElbow[0] - LWrist[0])
        l_y_dis = abs(LElbow[1] - LWrist[1])
        r_x_dis = abs(RElbow[0] - RWrist[0])
        r_y_dis = abs(RElbow[1] - RWrist[1])
        if l_y_dis / l_x_dis > 1.732 and LWrist[1] < LElbow[1]:
            raise_hand = 1
        if r_y_dis / r_x_dis > 1.732 and RWrist[1] < RElbow[1]:
            raise_hand = 1

        #######################################################

        # 算bbox,然后再检测并画一下表情,因为画表情不影响后面画,所以表情放在前面没问题
        bbox = human['box']
        bbox = [bbox[0], bbox[0] + bbox[2], bbox[1], bbox[1] + bbox[3]]  # xmin,xmax,ymin,ymax
        # 很神奇,原来的代码如果没有下面这一句的话,emotion里面会报bbox为空的错,一查,是因为第132帧的ymin变成了-1,很神奇,我在emotion里面给强行调个0
        # cv2.rectangle(img, (int(bbox[0]), int(bbox[2])), (int(bbox[1]), int(bbox[3])), color, 1)
        emotion(img, bbox, raise_hand)

        #######################################################

        # fall detection and draw bounding boxes
        neck = kp_preds[-1]
        lhip = kp_preds[11]
        rhip = kp_preds[12]
        mhip = (lhip + rhip) / 2

        x_dis = abs(mhip[0] - neck[0])
        y_dis = abs(mhip[1] - neck[1])
        if y_dis / x_dis < 0.6:
            cv2.rectangle(img, (int(bbox[0]), int(bbox[2])), (int(bbox[1]), int(bbox[3])), color, 1)
            font_scale = 1
            thickness = 1
            cv2.putText(img, 'fall', (int(bbox[0]), int((bbox[2] - 10))),
                        cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, thickness, cv2.LINE_AA)


        ######################################################

        # 下面这个是直接画出人的bbox的,我用蓝色框表示了,不用的时候就注释掉
        # bbox = human['box']
        # bbox = [bbox[0], bbox[0] + bbox[2], bbox[1], bbox[1] + bbox[3]]  # xmin,xmax,ymin,ymax
        ...
        # Draw keypoints
        # for n in range(kp_scores.shape[0]): # 因为要做表情,所以前5个点不画
